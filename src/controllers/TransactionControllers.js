...
class TransactionControllers{
    ...
    static async findSingleTransaction(req,res,next){
        try {
            const transaction = await TransactionServices.findTransactionById(req.params.id);
            if (!transaction){
                return res.status(409).json({
                    status: res.statusCode,
    ...
    static async updateTransaction(req,res,next){
        try {
            ...
            const getTransaction = await TransactionServices.findTransactionById(req.params.id);
            if(getTransaction){
                const updateTransaction = await TransactionServices.updateTransaction(req.params.id, updates);
    ...
    static async findCustomerTransactions(req, res, next){
        try {
            const customerTransactions = await TransactionServices.findAllCustomerTransactions(req.params.CustomerId);
    ...
    static async deleteTransaction(req,res, next){
        try {
            const transactionToDelete = await TransactionServices.findTransactionById(req.params.id);
            if (!transactionToDelete){
                ...
                const deletedTransaction = await TransactionServices.deleteTransaction(req.params.id);
                return res.status(200).json({
                    status: res.statusCode,
