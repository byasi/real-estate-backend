...
class PropertyControllers {
  static async registerProperty(req, res, next) {
    try {
      const propertyDetails = {
        image: `http://localhost:5000/properties/${req.file.filename}`,
        ...req.body,
      };

      const propertyExists = await PropertyServices.findPropertyByName(
        req.body.name
      );
  ...
  static async deleteProperty(req, res, next) {
    try {
      const propertyToDelete = await PropertyServices.getPropertyById(
        req.params.id
      );
      if (!propertyToDelete) {
        return res.status(409).json({
        ...
        const deletedProperty = await PropertyServices.deleteProperty(
          req.params.id
        );
        return res.status(200).json({
          status: res.statusCode,
  ...
  static async updateProperty(req, res, next) {
    try {
      ...
      if (getProperty) {
        const updateProperty = await PropertyServices.updateProperty(
          req.params.id,
          propertyToUpdate
        );
        if (updateProperty) {
